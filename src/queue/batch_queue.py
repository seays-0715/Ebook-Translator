...
JobStatus lifecycle (enforced by this module):
  ...
  PROCESSING → PAUSED | COMPLETED | COMPLETED_WITH_ERRORS | CANCELLED
      (final PROCESSING transition is owned by TranslationEngine.run())
  PAUSED → PENDING (resume) | CANCELLED
  COMPLETED → removable
  COMPLETED_WITH_ERRORS → PENDING (retry) | removable
  CANCELLED → removable (terminal; cannot resume)

PROCESSING jobs cannot be removed. Pause/cancel of PROCESSING require an
active TranslationEngine and only request_pause/request_stop; they do not
...
class BatchQueue:
    ...
    def emit(self, event: str, data: dict) -> None:
        """Public progress hook used by the UI watcher."""
    ...
    def remove(self, item_id: str, delete_job_data: bool = False) -> None:
        ...
        with self._lock:
            ...
            if delete_job_data and item.job_id:
                self.storage.delete_job(item.job_id)
    ...
    def _process_item(self, item: QueueItem) -> None:
        ...
        try:
            ...
            def progress(event: str, data: dict) -> None:
                payload = {**data, "item_id": item.item_id, "job_id": item.job_id}
