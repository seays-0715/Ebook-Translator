...
class App:
    ...
    def _build_nav(self) -> None:
        ...
        for key, label_key in [("convert", "convert"), ("translate", "translate"), ("glossary", "glossary")]:
            ctk.CTkButton(nav, text=_t(label_key), command=lambda k=key: self._show(k)).pack(fill="x", pady=4, padx=8)
        ctk.CTkButton(nav, text=_t("settings"), command=self._open_settings).pack(fill="x", pady=4, padx=8, side="bottom")
    ...
    def _build_convert_page(self) -> None:
        ...
        ctk.CTkButton(top, text=_t("open_file"), command=self._open_for_preview).pack(side="left", padx=4)
        ctk.CTkButton(top, text=_t("confirm_convert"), command=self._do_convert).pack(side="left", padx=4)
        ...
        for text_key, cmd in [("merge", self._op_merge), ("split", self._op_split), ("rename", self._op_rename), ("remove", self._op_remove)]:
            ctk.CTkButton(right, text=_t(text_key), command=cmd).pack(fill="x", pady=4, padx=8)
    ...
    def _handle_dropped_paths(self, paths: list[str]) -> None:
        for p in paths:
            ...
            if path.suffix.lower() in (".epub", ".txt") and path.is_file():
                ...
                return
        if paths:
            self._show_error("error_open_file", _t("error_open_file", detail="unsupported file type"))
    ...
    def _load_preview_path(self, path: Path) -> None:
        ...
        self.info_label.configure(
            text=_t("book_info", title=meta.title, author=meta.author, language=meta.language, chapters=len(result.book.chapters))
        )
        if result.warnings:
            self.convert_status.configure(text=_t("warnings_prefix", text="; ".join(result.warnings[:3])))
        else:
            self.convert_status.configure(text=_t("loaded_file", name=self._preview_source.name))
        self._show("convert")
    ...
    def _refresh_chapter_list(self) -> None:
        ...
        for ch in self._preview_book.chapters:
            ...
            label = _t("chapter_row", n=ch.order + 1, title=ch.title, blocks=len(ch.blocks))
            selected = ch.id == self._selected_chapter_id
            btn = ctk.CTkButton(
                self.chapter_list, text=label, anchor="w",
    ...
    def _op_merge(self) -> None:
        ...
        if idx + 1 >= len(ids):
            ...
            return
        try:
            self._preview_book = merge_adjacent(self._preview_book, cid, ids[idx + 1])
    ...
    def _op_rename(self) -> None:
        ...
        if not ch:
            return
        new_title = simpledialog.askstring(_t("rename_title"), _t("rename_prompt"), initialvalue=ch.title, parent=self.root)
        if not new_title:
            return
        try:
            self._preview_book = rename_chapter(self._preview_book, cid, new_title)
            self._refresh_chapter_list()
    ...
    def _op_remove(self) -> None:
        ...
        if not cid or not self._preview_book:
            return
        if not messagebox.askyesno(_t("confirm"), _t("remove_chapter_q"), parent=self.root):
            return
        try:
    ...
    def _do_convert(self) -> None:
        if not self._preview_book:
            messagebox.showinfo(_t("info"), _t("open_analyze_first"), parent=self.root)
            return
        try:
            out = filedialog.asksaveasfilename(
                parent=self.root, defaultextension=".epub",
                filetypes=[(_t("filetypes_epub"), "*.epub")],
                initialfile=f"{self._preview_source.stem}.epub" if self._preview_source else "output.epub",
            )
        except Exception as e:
    ...
    def _build_translate_page(self) -> None:
        ...
        ctk.CTkButton(row, text=_t("add_books"), command=self._add_translate).pack(side="left", padx=4)
        ctk.CTkButton(row, text=_t("start_translation"), command=self._start_queue).pack(side="left", padx=4)
        ctk.CTkButton(row, text=_t("pause"), command=self._pause_queue).pack(side="left", padx=4)
        ctk.CTkButton(row, text=_t("resume"), command=self._resume_queue).pack(side="left", padx=4)
        ...
        self.lbl_chapter = ctk.CTkLabel(prog, text=_t("progress_chapter", current=0, total=0), anchor="w")
        self.lbl_chapter.pack(fill="x")
        self.lbl_chunk = ctk.CTkLabel(prog, text=_t("progress_chunk", current=0, total=0), anchor="w")
        self.lbl_chunk.pack(fill="x")
        self.lbl_overall = ctk.CTkLabel(prog, text=_t("progress_overall", percent=0), anchor="w")
    ...
    def _add_translate(self) -> None:
        try:
            paths = filedialog.askopenfilenames(parent=self.root, filetypes=ebook_filetypes(_t("filetypes_ebook"), _t("filetypes_epub"), _t("filetypes_all")))
        except Exception as e:
            self._show_error("error_open_file", str(e))
            return
        ...
        self.translate_log.insert("end", _t("queued_books", count=len(self._translate_inputs)) + "\n")
    ...
    def _start_queue(self) -> None:
        ...
        cfg = JobConfig(
            ...
            model_identifier=self.settings.ai.model_identifier or self.settings.ai.model,
        ...
        def on_progress(event, data):
            def ui():
                if event == "chunk_done":
                    ...
                    self.progress_label.configure(text=_t("progress_job_line", job_id=job_id, completed=completed, total=total, status=data.get("status")))
                elif event in ("job_done", "exported", "item_exported"):
                    self.translate_log.insert("end", _t("progress_status", status=str(data.get("status", event))) + "\n")
                elif event == "queue_finished":
                    self.translate_log.insert("end", _t("queue_finished") + "\n")
                    self.progress_label.configure(text=_t("queue_finished"))
                    after_completion_action(self.settings.output.after_completion, output_folder=self._translate_output_dir)
            self.root.after(0, ui)

        try:
            q = BatchQueue(storage=self.storage, work_root=data_dir() / "jobs", config=cfg, on_progress=on_progress)
    ...
    def _build_glossary_page(self) -> None:
        ...
        row = ctk.CTkFrame(page)
        row.pack(fill="x", pady=4)
        ctk.CTkButton(row, text=_t("create"), command=self._gloss_create).pack(side="left", padx=4)
        ctk.CTkButton(row, text=_t("build_from_pair"), command=self._gloss_build).pack(side="left", padx=4)
        ctk.CTkButton(row, text=_t("refresh"), command=self._gloss_refresh).pack(side="left", padx=4)
    ...
    def _gloss_create(self) -> None:
        name = simpledialog.askstring(_t("glossary_name_prompt"), _t("glossary_name_prompt"), initialvalue=_t("glossary_name_default"), parent=self.root)
        if not name:
            return
        try:
            g = self.glossary_store.create(name)
            messagebox.showinfo(_t("info"), _t("glossary_created", id=g.glossary_id[:8]), parent=self.root)
            self._gloss_refresh()
        except Exception as e:
    ...
    def _gloss_build(self) -> None:
        try:
            src = filedialog.askopenfilename(parent=self.root, title=_t("original_book"), filetypes=ebook_filetypes(_t("filetypes_ebook"), _t("filetypes_epub"), _t("filetypes_all")))
            if not src:
                return
            tgt = filedialog.askopenfilename(parent=self.root, title=_t("official_translation"), filetypes=ebook_filetypes(_t("filetypes_ebook"), _t("filetypes_epub"), _t("filetypes_all")))
            if not tgt:
                return
            name = simpledialog.askstring(_t("glossary_build_name"), _t("glossary_build_name"), parent=self.root)
            if not name:
                return
            ...
            msg = _t("glossary_build_result", id=g.glossary_id[:8], candidates=len(result.candidates), pairs=len(result.pairs))
            if result.needs_manual_alignment:
                msg += "\n" + _t("glossary_manual_align", message=result.message or "")
    ...
    def _gloss_refresh(self) -> None:
        ...
        try:
            for gid in self.glossary_store.list_ids():
                ...
                self.gloss_box.insert("end", _t("glossary_row", id=gid[:8], name=g.name, version=g.version, entries=len(g.entries), confirmed=confirmed) + "\n")
        except Exception as e:
            log.exception("glossary refresh failed")
            self.gloss_box.insert("end", str(e) + "\n")

    def _option_labels(self, codes, label_for):
        labels, l2c, c2l = [], {}, {}
        for code in codes:
            lab = label_for(code)
            labels.append(lab)
    ...
    def _open_settings(self) -> None:
        ...
        def section(title_key: str) -> None:
            ctk.CTkLabel(scroll, text=_t(title_key), font=("", 14, "bold")).pack(anchor="w", pady=(12, 4))

        def field(key: str, label_key: str, value: str) -> None:
            row = ctk.CTkFrame(scroll)
            row.pack(fill="x", pady=2)
            ctk.CTkLabel(row, text=_t(label_key), width=180, anchor="w").pack(side="left")
            e = ctk.CTkEntry(row, width=280)
            e.insert(0, value)
            e.pack(side="left", padx=4)
            entries[key] = e

        def dropdown(key: str, label_key: str, codes, current: str, label_for) -> None:
        ...
        field("model_identifier", "label_model_id", self.settings.ai.model_identifier or "")
        ...
        def _style_label(code: str) -> str:
            return {"auto": _t("style_auto"), "natural": _t("style_natural"), "faithful": _t("style_faithful"), "literary": _t("style_literary"), "light_novel": _t("style_light_novel"), "custom": _t("style_custom")}.get(code, code)

        def _after_label(code: str) -> str:
            return {"nothing": _t("after_nothing"), "sleep": _t("after_sleep"), "shutdown": _t("after_shutdown"), "open_folder": _t("after_open_folder")}.get(code, code)

        def _iface_label(code: str) -> str:
            return {"": _t("lang_auto"), "zh-HK": _t("lang_zh_hk"), "zh-TW": _t("lang_zh_tw"), "en": _t("lang_en")}.get(code, code)

        dropdown("source_language", "label_source_lang", _SOURCE_LANG_CODES, self.settings.translation.source_language, _src_label)
        dropdown("target_language", "label_target_lang", _TARGET_LANG_CODES, self.settings.translation.target_language, lambda c: c)
        dropdown("style", "label_style", _STYLE_CODES, self.settings.translation.style, _style_label)
        field("chunk_target_tokens", "label_chunk_tokens", str(self.settings.translation.chunk_target_tokens))
        field("carry_over_paragraphs", "label_carry_over", str(self.settings.translation.carry_over_paragraphs))
        ctk.CTkLabel(scroll, text=_t("label_prompt"), anchor="w").pack(anchor="w", pady=(8, 2))
        ...
        ctk.CTkLabel(scroll, text=_t("hint_prompt"), anchor="w").pack(anchor="w", padx=4)

        section("settings_retry")
        field("timeout_seconds", "label_timeout", str(self.settings.ai.timeout_seconds))
        field("retry_count", "label_retry_count", str(self.settings.ai.retry_count))
        field("retry_delay_seconds", "label_retry_delay", str(self.settings.ai.retry_delay_seconds))
        field("request_interval_seconds", "label_request_interval", str(self.settings.ai.request_interval_seconds))
        field("endpoint_fail_threshold", "label_endpoint_fail", str(self.settings.ai.endpoint_fail_threshold))

        section("settings_output")
        dropdown("after_completion", "label_after_completion", _AFTER_CODES, self.settings.output.after_completion, _after_label)

        section("settings_interface")
        dropdown("interface_language", "label_interface_lang", _INTERFACE_LANG_CODES, self.settings.interface_language or "", _iface_label)

        section("settings_advanced")
        field("max_image_edge", "label_max_image_edge", str(self.settings.max_image_edge))

        def _get(key: str) -> str:
            w = entries[key]
            if key in option_maps:
                return option_maps[key].get(w.get(), w.get())  # type: ignore
            if hasattr(w, "get") and "Textbox" not in type(w).__name__:
                return w.get().strip()  # type: ignore
            return w.get("1.0", "end").strip()  # type: ignore
        ...
        def save() -> None:
            ...
            try:
                ...
                self.settings.ai.retry_delay_seconds = _get_float("retry_delay_seconds", 2.0)
                self.settings.ai.request_interval_seconds = _get_float("request_interval_seconds", 0.5)
                self.settings.ai.endpoint_fail_threshold = _get_int("endpoint_fail_threshold", 3)
                ...
                self.settings.translation.chunk_target_tokens = _get_int("chunk_target_tokens", 1000)
                self.settings.translation.carry_over_paragraphs = _get_int("carry_over_paragraphs", 2)
                ...
                self._show_error("error", str(e))
                return
            lang_changed = (self.settings.interface_language or "") != (prev_lang or "")
            win.destroy()
            if lang_changed:
                self._prompt_restart_for_language()
            else:
                messagebox.showinfo(_t("info"), _t("settings_saved"), parent=self.root)
    ...
    def _prompt_restart_for_language(self) -> None:
        ...
        dlg.grab_set()
        ctk.CTkLabel(dlg, text=_t("restart_prompt"), wraplength=380, justify="left").pack(padx=16, pady=16)
        ...
        row.pack(pady=8)
        ctk.CTkButton(row, text=_t("restart_now"), command=do_restart).pack(side="left", padx=8)
        ctk.CTkButton(row, text=_t("restart_later"), command=do_later).pack(side="left", padx=8)
