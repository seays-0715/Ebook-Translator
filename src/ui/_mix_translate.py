...
from src.ui._mix_translate_prompt_save import TranslatePromptSaveMixin
...
class TranslateMixin(TranslatePromptSaveMixin):

    @staticmethod
    def _selection_label_for_persisted_language(raw_code: str | None) -> tuple[str, str | None]:
        """Map persisted language code to OptionMenu label.

        Valid registry codes → display label, no error.
        Invalid / empty codes → empty label + error message (never substitute
        another language such as Japanese or Traditional Chinese).
        """
        ...
        try:
            ...
            return "", str(exc)
        return language_code_to_label(code), None
    ...
    def _build_translate_page(self) -> None:
        ...
        src_label, src_err = self._selection_label_for_persisted_language(
            ts.source_language
        )
        tgt_label, tgt_err = self._selection_label_for_persisted_language(
            ts.target_language
        )
        invalid_languages: list[str] = []
        if src_err is not None:
            invalid_languages.append(f"source: {src_err}")
        if tgt_err is not None:
            invalid_languages.append(f"target: {tgt_err}")

        ctk.CTkLabel(cfg_row, text=_t("label_source_lang")).pack(side="left", padx=4)
        self._tr_src_var = ctk.StringVar(value=src_label)
        ctk.CTkOptionMenu(
        ...
        self._tr_tgt_var = ctk.StringVar(value=tgt_label)
        ctk.CTkOptionMenu(
        ...
        ctk.CTkButton(
            ...
            text=_t("save"),
            width=80,
            command=self._save_translate_prompt,
